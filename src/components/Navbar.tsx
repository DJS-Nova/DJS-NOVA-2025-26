  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[999] pointer-events-auto">
      <div className="relative backdrop-blur-xl bg-white/10 text-white text-4xl font-light tracking-widest shadow-[0_0_40px_rgba(255,255,255,0.15)]">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center py-2 px-6 h-14 text-white relative z-10">
          ...
          <nav className="hidden md:block">
            <ul className="flex gap-10 text-[15px] tracking-[0.2em]">
              {routes.map(({ name, url, onblank }) => {
                const isActive = pathname === url;
                return (
                  <li key={name} className="relative group">
                    <Link href={url} className={`flex items-center gap-2 transition-all duration-300 ${isActive ? "text-white" : "text-gray-400 hover:text-white"}`} target={onblank ? "_blank" : "_self"}>
                      {name}
                    </Link>
        ...
        {/* Mobile menu */}
        <div
          className={`fixed top-14 left-0 w-full z-[9998] backdrop-blur-2xl bg-slate-900/90 border-t border-white/10 transition-all duration-500 ease-in-out overflow-hidden ${menuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
            }`}
        >
              ...
              return (
                <li key={name}>
                  <Link
                    ...
                    className={`flex items-center gap-2 transition-all duration-300 ${isActive ? "text-amber-400" : "text-gray-400 hover:text-white"
                      }`}
                  >
                    {name}
                  </Link>
